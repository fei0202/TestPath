...
public class LimelightHelpers {
    ...
    // 1. 硬體與基本連接
    // 確保 Limelight 已正確連接至機器人的 NetworkTables 網路，並可以透過 Limelight
    // 的網頁介面（http://limelight.local:5801/）訪問與調整。

    // 2. 設定 Limelight 進行 AprilTag 偵測
    // 開啟 Limelight 的網頁介面。
    // 切換到 "Pipeline" 設定。
    // 選擇 AprilTag 模式：
    // Pipeline Type → "AprilTag"
    // 選擇適當的 Tag Family（一般為 "16h5" 或 "36h11"，FRC 2023+ 使用 "36h11"）
    // 調整曝光 (Exposure) 和增益 (Gain)，確保相機能清楚看到 AprilTag。
    // 確認 Limelight 可以在 "Results" 頁面顯示 AprilTag ID、X/Y 偏移等資訊

    // 例：根據偏移自動旋轉機器人

    // double xOffset = xEntry.getDouble(0);
    // double kp = 0.02; // 控制增益 (可根據需求調整)
    // double turnSpeed = kp * xOffset;

    // driveTrain.arcadeDrive(0, -turnSpeed); // 旋轉機器人對齊 AprilTag

    // 自動導航 (Autonomous Mode)：使用 AprilTag ID 定位不同區域，讓機器人自動導航至目標位置。
    // 機械手臂對齊 (Aligning Mechanisms)：讓機械手臂或升降機精確對齊 AprilTag。
    // 自動停車 (Docking System)：讓機器人自動對齊 AprilTag 進行停車或交付物品。
